...
namespace DesasterAlleviationFund.Controllers
{
    ...
    {
        string conn = "Server=tcp:st10106500.database.windows.net,1433;Initial Catalog=DjPromoWebsite;Persist Security Info=False;User ID=djadmin;Password=Jessicalulama14;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;";
        ...
        public IActionResult Registration(Registration register)
        {
            if (register != null)
            {
                if (!string.IsNullOrEmpty(register.Email) &&
                    !string.IsNullOrEmpty(register.First_Name) &&
                    !string.IsNullOrEmpty(register.Last_Name) &&
                    !string.IsNullOrEmpty(register.Password))
                {
                    ...
                    using (SqlConnection con = new SqlConnection(conn))
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand(query, con))
                        {
                            ...
                            cmd.Parameters.AddWithValue("@Email", register.Email.ToString());
                            cmd.Parameters.AddWithValue("@First_Name", register.First_Name.ToString());
                            cmd.Parameters.AddWithValue("@Last_Name", register.Last_Name.ToString());
        ...
        [HttpPost]
        public IActionResult Monentart_Donations(Monetory monetory)
        {
            ...
            using (SqlConnection con = new SqlConnection(conn))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    ...
                    if (monetory.DonorName == "other")
                    {
                        cmd.Parameters.AddWithValue("@DonorName", monetory.DonorNameInput);
                    }
                    else if (monetory.DonorName == "Anonymous")
                    {
                    ...
                    cmd.Parameters.AddWithValue("@Date", monetory.Date);
                    cmd.Parameters.AddWithValue("@Amount", monetory.Amount);
        ...
        public IActionResult ListMonetary()
        {
            DataTable dt = new DataTable();
            using (SqlConnection con = new SqlConnection(conn))
            {
        ...
        public IActionResult Good_Donations(Good good)
        {
            ...
            {
                con.Open();
                string query = "INSERT INTO Good (Email, DonarName, Date, numberOfItem, Category, Description) VALUES (@Email, @DonarName, @Date, @numberOfItem, @Category, @Description)";
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    string don = "Anonymous";
                    cmd.CommandType = CommandType.Text;
                    ...
                    cmd.Parameters.AddWithValue("@numberOfItem", good.numberOfItem);
        ...
        public IActionResult EditGoods(int id, Good good)
        {
            try
            {
                ...
                if (dt.Rows.Count == 1)
                {
                    ...
                    good.numberOfItem = Convert.ToInt32(dt.Rows[0][4].ToString());
        ...
        public IActionResult EditGoods(Good good)
        {
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(conn))
                {
                    string query = "UPDATE Good SET Email = @Email, DonarName = @DonarName, Date = @Date, numberOfItem = @numberOfItem, Category = @Category, Description = @Description  WHERE GoodId = @GoodId";
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, con))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@Date", good.Date);
                        cmd.Parameters.AddWithValue("@NumItems", good.numberOfItem);
                        cmd.Parameters.AddWithValue("@Category", good.Category);
                        cmd.Parameters.AddWithValue("@Description", good.Description);
